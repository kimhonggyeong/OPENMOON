        ...
        for row in rows:
            writer.writerow(
                [
                    ...
                    row["block_index"],
                    get_column_letter(
                        row["block_start_column"]
                    ),
                    get_column_letter(
                        row["block_end_column"]
                    ),
...
def build_database(
    ...
    try:
        for index, file_path in enumerate(
            ...
            try:
                ...
                for quotation in quotations:
                    ...
                    print(
                        f"    - "
                        f"{quotation.sheet_name} / "
                        f"블록 {quotation.block_index} / "
                        f"{start_letter}:{end_letter} / "
                        f"{quotation.parse_status} / "
                        f"품목 {len(quotation.items)}개"
                    )

                    if quotation.parse_error:
                        print(
                            "      사유:",
                            quotation.parse_error
                        )
        ...
        quotation_count = connection.execute(
        ...
        print("=" * 80)
        print("DB 구축 완료")
        print("처리 파일:", processed_files)
        print("건너뛴 파일:", skipped_files)
        print("실패 파일:", failed_files)
        print("DB 파일 수:", file_count)
        print("DB 견적 수:", quotation_count)
        print("DB 품목 수:", item_count)
        print("검토 필요:", review_count)
        print("DB 위치:", database_path.resolve())
        print(
            "검토 CSV:",
    ...
    try:
        ...
        if (
            ...
            dimension_sql = """
                CASE
                    WHEN
                        qi.width_mm IS NOT NULL
                        AND qi.height_mm IS NOT NULL
                    THEN
                        MIN(
                            ABS(qi.width_mm - ?)
                            + ABS(qi.height_mm - ?),
        ...
        rows = connection.execute(
            ...
            WHERE
                qi.normalized_product LIKE ?
                OR qi.product_name LIKE ?
                OR qi.specification_raw LIKE ?
                OR qi.detail_text LIKE ?
        ...
        for index, row in enumerate(
            ...
            print(
                "규격:",
                row["specification_raw"]
                or "미확인"
            )

            if (
                row["width_mm"] is not None
                and row["height_mm"] is not None
            ):
                print(
                    "크기:",
                    f"{row['width_mm']} × "
...
def show_summary(
    ...
    try:
        ...
        print("=" * 70)
        print("DB 요약")
        print("파일 수:", file_count)
        print("견적 수:", quotation_count)
        print("품목 수:", item_count)
        print("=" * 70)
