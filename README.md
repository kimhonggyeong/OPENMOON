...
OPENMOON/
│
├── assets/                     # 로고, 직인, 아이콘 등 (보안상 업로드하지 않음)
├── quotation_drafts/           # 생성된 견적서 저장 폴더
├── quotation_files/            # 기존 견적서 샘플(xlsx) (보안상 업로드하지 않음)
│
├── .env                        # 환경설정 (보안상 업로드하지 않음)
├── 단가표.xlsx                 # 회사 단가표 원본 (보안상 업로드하지 않음)
│
