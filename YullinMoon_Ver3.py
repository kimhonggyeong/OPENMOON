...
# 단가표 검색용 SQLite DB
PRICE_TABLE_DATABASE_PATH = Path(
    "price_table.db"
)
...
TEMPLATE_SHEET_NAME = "Main_Sheet"

if not QUOTATION_DATABASE_PATH.exists():
    raise FileNotFoundError(
        "기존 견적서 DB를 찾을 수 없습니다.\n"
        f"필요한 위치: {QUOTATION_DATABASE_PATH.resolve()}"
    )

if not PRICE_TABLE_DATABASE_PATH.exists():
    raise FileNotFoundError(
        "단가표 DB를 찾을 수 없습니다.\n"
        f"필요한 위치: "
        f"{PRICE_TABLE_DATABASE_PATH.resolve()}"
    )
...
def normalize_order_related(
    ...
    """
    GPT가 quotation_request 또는 inquiry로 분류하면서
    is_order_related=False를 반환하는 불일치를 보정한다.

    품목이 있고 주문·견적·제작 문의에 해당하면
    견적서 초안 생성 대상으로 처리한다.
    """
    ...
    if (
        analysis.category in order_related_categories
        and len(analysis.items) > 0
    ):
...
17. is_order_related는 확정 주문만 의미하지 않습니다.
18. order, quotation_request 또는 제작·인쇄 관련 inquiry이고
    구체적인 품목이 하나 이상 확인되면 is_order_related는 true입니다.
19. 견적서 요청, 시안 요청, 제작 가능 여부 문의도
    품목이 확인되면 is_order_related를 true로 반환하세요.
20. advertisement, shipping, payment, other는
    is_order_related를 false로 반환하세요.
21. category가 quotation_request인데 품목이 있다면
    is_order_related를 false로 반환하지 마세요.
22. 여러 품목이 언급되면 한 품목에 합치지 말고 각각 분리하세요.
    예: "현수막, 배너 제작"은 현수막과 배너 두 품목입니다.
...
def print_price_decisions(
    ...
    for index, decision in enumerate(
        ...
        print()
        print(
            f"{index}. {decision.product_name}"
        )

        if decision.unit_price is not None:
            print(
                "   단가:",
                f"{decision.unit_price:,}원"
            )
        else:
            print(
                "   단가: 미확인"
...
def process_mail(
    ...
    # GPT 분류와 is_order_related 값이 충돌하는 경우 보정
    ...
    # 주문 관련 메일의 각 품목 가격 검색
    price_decisions: list[PriceDecision] = []

    if (
            analysis.is_order_related
            and analysis.items
    ):
        print()
        print(
        ...
        price_decisions = apply_prices(
            items=analysis.items,
            customer_organization=recipient,
            ...
            price_database_path=(
                PRICE_TABLE_DATABASE_PATH
            )
        )

        # 모든 품목 금액이 확인된 경우에만 총액 확정
    ...
    if price_decisions:
        print_price_decisions(
            price_decisions
        )

    # 확정 주문인 경우에만 견적서 생성
    # 주문·견적 관련 메일이며 품목이 있으면 초안 생성
    if (
            analysis.is_order_related
            and analysis.items
    ):
